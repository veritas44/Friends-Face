    ...
    protected Activity doInBackground(Activity[] activities) {
        try {
            if (!activities[0].isDestroyed()) {
                ...
                if (name == null)
                    name = document.getElementsByClass("br").select("img").eq(0).attr("alt");
                if (cover == null)
                    cover = document.getElementById("profile_cover_photo_container").select("img").eq(0).attr("src");
                if(propic == null)
                    propic = document.getElementsByClass("br").select("img").eq(0).attr("src");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
